...
- [Development guidelines](#development-guidelines)
    - [General Vertexcoin Core](#general-vertexcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Vertexcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Vertexcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/vertexcoin/vertexcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_vertexcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_vertexcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/vertexcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_vertexcoin.coverage/index.html`.
```
...
To profile a running vertexcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep vertexcoind` -- sleep 60
```
...
Vertexcoin Core can be compiled with various "sanitizers" enabled, which add
...
file descriptors it will cause problems with Vertexcoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Vertexcoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Vertexcoin Core, the wallet was always
